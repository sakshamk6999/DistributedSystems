...
def handle_product_search(client_socket: socket.socket, args):
    if not current_session['is_logged_in'] or current_session['session_id'] == None:
        print("Invalid action. You should be logged in first.")
        return
    ...
    if res_object['status'] == "OK":
        print(f"Here are the items: {res_object['items']}")
    else:
        print("ERROR in response")
        print(res_object['message'])

def handle_add_item_to_cart(client_socket: socket.socket, args):
    if not current_session['is_logged_in'] or current_session['session_id'] == None:
        print("Invalid action. You should be logged in first.")
        return
...
def handle_client_cmd(client_socket: socket.socket, received_cmd):
    ...
    if command == 'REGISTER':
        ...
        handle_product_search(client_socket, args)
    elif command == "ADD_ITEM":
        handle_add_item_to_cart(client_socket, args)
...
def main():
    ...
    try:
        server_host = os.getenv("BUYER_SERVER_HOST", "0.0.0.0")
        server_port = int(os.getenv("BUYER_SERVER_PORT", 50000))
